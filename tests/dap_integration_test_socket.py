...
"""
Integration test for DAP (Debug Adapter Protocol) implementation in frobd (Socket mode)

Usage: python dap_integration_test.py <path/to/frobd> <path/to/image_file.t3>

This test script performs an end-to-end integration test of the DAP (Debug Adapter Protocol) implementation in frobd. It launches frobd with the specified TADS 3 image and connects to the DAP socket to send requests and validate responses.
...
def start_debug_session():
    ...
    socket_path = os.path.join(socket_dir, "frobd_dap.sock")
    ...
    for _ in range(50):
        ...
        proc.terminate()
        raise RuntimeError("DAP socket was not created by frobd")

    sock = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
    sock.connect(socket_path)
    sock.settimeout(8)

    return proc, sock, socket_dir
